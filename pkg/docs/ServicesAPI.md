...
> OperationResult UpdateServiceFromManifest(ctx, id).ServiceManifestUpdateInput(serviceManifestUpdateInput).Execute()
...
func main() {
	id := int32(56) // int32 | 
	serviceManifestUpdateInput := *openapiclient.NewServiceManifestUpdateInput("ManifestYaml_example") // ServiceManifestUpdateInput | 

	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ServicesAPI.UpdateServiceFromManifest(context.Background(), id).ServiceManifestUpdateInput(serviceManifestUpdateInput).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ServicesAPI.UpdateServiceFromManifest``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `UpdateServiceFromManifest`: OperationResult
	fmt.Fprintf(os.Stdout, "Response from `ServicesAPI.UpdateServiceFromManifest`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiUpdateServiceFromManifestRequest struct via the builder pattern


Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------

 **serviceManifestUpdateInput** | [**ServiceManifestUpdateInput**](ServiceManifestUpdateInput.md) |  | 
