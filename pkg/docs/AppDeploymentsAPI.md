...
> AppDeployment CancelAppDeployment(ctx, id).Execute()
...
func main() {
	...
	resp, r, err := apiClient.AppDeploymentsAPI.CancelAppDeployment(context.Background(), id).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AppDeploymentsAPI.CancelAppDeployment``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `CancelAppDeployment`: AppDeployment
	fmt.Fprintf(os.Stdout, "Response from `AppDeploymentsAPI.CancelAppDeployment`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiCancelAppDeploymentRequest struct via the builder pattern
